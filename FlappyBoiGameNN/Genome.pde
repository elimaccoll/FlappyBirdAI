class Genome {
  ...
  ArrayList<Node> network = new ArrayList<Node>(); // ordered list of nodes for NN
  ...
  boolean randomConnectionNodesAreShit(int r1, int r2) {
    ...
    if (nodes.get(r1).isConnected(nodes.get(r2))) return true; //if the nodes are already connected
